...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 06-04-2024 a las 09:32:09
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_MODIFICAR_CONTRA_USUARIO` (IN `IDUSU` INT, IN `CONTRA` VARCHAR(65))   UPDATE  usuario SET
usuario.Usr_Pswd=CONTRA$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_MODIFICAR_DATOS_USUARIO` (IN `IDUSU` INT, IN `NOMBRE` VARCHAR(40), IN `APELLIDO` VARCHAR(40), IN `IDROL` INT(2), IN `CORREO` VARCHAR(255), IN `TEL` VARCHAR(10))   UPDATE usuario SET
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_MODIFICAR_ESTADO_USUARIO` (IN `IDUSU` INT, IN `ESTADOUSU` VARCHAR(8))   UPDATE usuario SET
usuario.Est_Id=ESTADOUSU
WHERE usuario.Usr_Id=IDUSU$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_PRUEBA` (IN `CC` INT(10))   BEGIN 
DECLARE  CANTIDAD INT;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_REGISTRAR_USUARIO` (IN `CC` INT(10), IN `ROL` INT(1), IN `NOMBRE` VARCHAR(40), IN `APELLIDO` VARCHAR(40), IN `CONTRA` VARCHAR(65), IN `CORREO` VARCHAR(255), IN `TEL` VARCHAR(10))   BEGIN DECLARE  CANTIDAD INT;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_VERIFICAR_USUARIO` (IN `USUARIO` INT(10))   SELECT 
usuario.Usr_Id,
usuario.Rol_Id,
...
(444, 3, 'Vale', 'Tobar', '$2y$10$69BnIe6zHjdW.Eb5vmuj4eanocH6CMyzTm4WBzOdkFYvslzhiLUcO', 'Vale@hotmail.com', '444445', 2),
(9993, 2, 'Liliana Jineth', 'Zamora Rodriguez', '$2y$10$69BnIe6zHjdW.Eb5vmuj4eanocH6CMyzTm4WBzOdkFYvslzhiLUcO', 'lilianita993@hotmail.com', '3145637222', 1),
(99234755, 2, 'Yohan Stid', 'Garcia Tobar', '$2y$10$69BnIe6zHjdW.Eb5vmuj4eanocH6CMyzTm4WBzOdkFYvslzhiLUcO', 'yohan@medico.co', '3245524425', 1),
(1095832411, 1, 'Albert David ', 'Sanchez', '$2y$10$69BnIe6zHjdW.Eb5vmuj4eanocH6CMyzTm4WBzOdkFYvslzhiLUcO', 'albert@hotmail.com', '3261983939', 1),
(1110577198, 1, 'Juan Sebastian ', 'Tobar Rojas', '$2y$10$69BnIe6zHjdW.Eb5vmuj4eanocH6CMyzTm4WBzOdkFYvslzhiLUcO', 'juanseone28@gmail.com', '3118345736', 1);
